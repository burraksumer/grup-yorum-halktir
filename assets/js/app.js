...
const AudioPlayer = {
	mounted() {
		...
		this.audio.addEventListener('timeupdate', () => {
			try {
				...
				if (
					...
					if (
						!this.lastDuration ||
						Math.abs(this.lastDuration - this.audio.duration) > 0.5
					) {
				...
				if (currentTrackId) {
					const currentTimeElements = document.querySelectorAll(
						`[data-track-current-time="${currentTrackId}"]`
					);
					...
					if (this.audio.duration && !isNaN(this.audio.duration)) {
						const durationElements = document.querySelectorAll(
							`[data-track-duration="${currentTrackId}"]`
						);
		...
		this.audio.addEventListener('loadedmetadata', () => {
			try {
				...
				if (
					...
					const trackDurationElements = document.querySelectorAll(
						`[data-track-duration="${currentTrackId}"]`
					);
		...
		this.observer.observe(this.el, {
			attributes: true,
			attributeFilter: ['src', 'data-volume'],
		});
...
const VolumeSlider = {
	...
	mounted() {
		...
		if (
			this.audio &&
			initialSliderValue !== undefined &&
			!isNaN(initialSliderValue)
		) {
		...
		if (this.isVisible()) {
			...
			if (tracksData) {
				...
				const needsPreload = tracks.some((track) => {
					...
					return !(
						cachedDurations[trackId] ||
						cachedDurations[trackId.toString()] ||
						knownDurations[trackId] ||
						knownDurations[trackId.toString()]
					);
				});
